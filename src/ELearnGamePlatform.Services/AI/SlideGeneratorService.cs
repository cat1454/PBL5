...
""";
    ...
    public string RenderDeckHtml(SlideDeck deck, IReadOnlyList<SlideItem> items)
    {
        ...
        html.AppendLine($"@import url('https://fonts.googleapis.com/css2?family=Lexend:wght@300..900&display=swap');{themeCss} :root{{--slide-font-family:'Lexend','Noto Sans',system-ui,-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;}} body{{margin:0;font-family:var(--slide-font-family);background:var(--deck-bg);color:var(--deck-text);}} .page{{width:min(1080px,calc(100vw - 32px));margin:0 auto;padding:32px 0 64px;font-family:inherit;}} .hero{{padding:40px 8px 12px;}} .hero h1{{margin:0;font-family:inherit;font-size:clamp(2.4rem,6vw,4.9rem);line-height:.94;letter-spacing:-.05em;}} .hero p{{max-width:780px;color:var(--deck-muted);line-height:1.8;}} .slides{{display:grid;gap:22px;margin-top:24px;}} .slide{{background:var(--card-bg);border:1px solid var(--card-border);border-radius:30px;padding:30px;box-shadow:0 18px 48px rgba(15,23,42,.12);page-break-inside:avoid;font-family:inherit;}} .slide-meta{{display:flex;justify-content:space-between;font:inherit;font-size:.86rem;font-weight:600;text-transform:uppercase;letter-spacing:.08em;color:var(--deck-soft);}} .slide h2{{margin:10px 0 0;font-family:inherit;font-size:clamp(1.7rem,4.5vw,3rem);line-height:1.05;letter-spacing:-.03em;}} .slide p.sub{{margin:0;color:var(--deck-muted);line-height:1.7;}} .goal{{display:inline-block;margin-top:12px;padding:9px 14px;border-radius:999px;background:var(--goal-bg);color:var(--goal-text);font:inherit;font-size:.92rem;font-weight:500;}} .body{{margin-top:18px;font-family:inherit;}} .body ul{{margin:0;padding-left:20px;display:grid;gap:10px;}} .body li,.body p,.notes p{{line-height:1.7;}} .notes{{border-top:1px solid var(--notes-border);margin-top:16px;padding-top:14px;color:var(--deck-soft);font:inherit;font-size:.94rem;}} .slide-title{{min-height:420px;display:grid;align-content:end;background:var(--title-bg);}} .slide-title h2{{font-size:clamp(2.8rem,7vw,5rem);}} .slide-sectiondivider{{background:var(--divider-bg);color:var(--divider-text);}} .slide-sectiondivider p.sub,.slide-sectiondivider .slide-meta,.slide-sectiondivider .notes{{color:var(--divider-muted);}} .slide-highlight{{background:var(--highlight-bg);}} .slide-quote .body p{{font-size:1.16rem;font-style:italic;}} .slide-stat .body li{{font-weight:700;}} @media print{{body{{background:#fff}}.page{{width:100%;padding:0}}.hero{{display:none}}.slide{{box-shadow:none;margin-bottom:18px}}}}");
